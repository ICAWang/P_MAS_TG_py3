...
class ltl_planner(object):
	def __init__(self, ts, hard_spec, soft_spec):
		...
		self.Time = 0
		self.cur_pose = None
		self.trace = [] # record the regions been visited 记录被访问的区域
		self.traj = [] # record the full trajectory 记录全部的轨迹
		self.opt_log = [] # record [(time, prefix, suffix, prefix_cost, suffix_cost, total_cost)]
		self.com_log = [] # record [(time, no_messages)]
